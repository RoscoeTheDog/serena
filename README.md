...
See the CONTRIBUTING.md file for guidelines on adding new language support.
...
3. In the `project.yml` file for each project, stored in `~/.serena/projects/{project-id}/project.yml`.
   This holds project-level configuration that is used whenever that project is activated.
   This file will be autogenerated when you first use Serena on that project, but you can also
...
project, a configuration file will be generated in `~/.serena/projects/{project-id}/project.yml`.
You can adjust this configuration, e.g., by changing the project name or other options.
...
**Memories** are files stored in `~/.serena/projects/{project-id}/memories/`,
...
Every file in the `memories/` directory is a memory file.
...
It is also relatively straightforward to add support for a new programming language (see CONTRIBUTING.md for guidelines).
