...
**Executed**: 2025-12-23
**Remediation Story**: 3.i.1

### Changes Applied

#### context_mode.py
1. ✅ Added `SerenaPaths` to import from `serena_config`
2. ✅ Verified `data.pop("single_project", None)` preserved (line 149)
3. ✅ Verified deprecation mapping preserved (lines 172-177)
4. ✅ All USER_*_DIR constants preserved in imports

#### serena_config.py
1. ✅ Added `import dataclasses` and `from enum import Enum`
2. ✅ Enhanced `SerenaPaths.__init__` with SERENA_HOME env var support
3. ✅ Added `user_prompt_templates_dir`, `user_contexts_dir`, `user_modes_dir` properties
4. ✅ Verified `rel_path_to_project_yml` preserved (centralized storage)
5. ✅ Verified `load` method preserved (centralized storage)
6. ✅ All fork enhancements intact

### Verification

- [x] Python syntax check passed
- [x] SerenaPaths import works
- [x] Fork enhancement: single_project removal preserved
- [x] Fork enhancement: ide-assistant deprecation mapping preserved
- [x] Fork enhancement: centralized storage pattern preserved
- [x] Merge strategy document marked as EXECUTED

### Skipped

- **SERENA_FILE_ENCODING import**: Constant doesn't exist in fork's constants.py, not used in code
- **Utility imports** (get_dataclass_default, ask_yes_no): Functions don't exist in fork yet
- **Unit tests**: Python version mismatch (3.13 vs required 3.11-3.12)

### Notes

- Merge executed per approved merge strategy document (`.claude/sprint/plans/3-merge-strategy.md`)
- All P0 acceptance criteria met
- Syntax validation passed for both files
- Fork enhancements verified preserved
- Ready for integration testing in proper environment
